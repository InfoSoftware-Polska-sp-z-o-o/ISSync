...
const intlMessages = defineMessages({
  ...
  startExternalScreenLabel: {
    id: 'app.actionsBar.actionsDropdown.shareExternalScreen',
    description: 'Start sharing external screen button',
  },
  stopExternalScreenLabel: {
    id: 'app.actionsBar.actionsDropdown.stopShareExternalScreen',
    description: 'Stop sharing external screen button',
  },
  selectRandUserLabel: {
...
class ActionsDropdown extends PureComponent {
  constructor(props) {
    ...
    this.handleExternalScreenClick = this.handleExternalScreenClick.bind(this);
  ...
  getAvailableActions() {
    ...
    const isSharingScreen = Session.get('isExternalIsEnabled') === true ? true: false;
    const isLogged = Cookies.get("us") && Cookies.get("us") !== "" && Cookies.get("pass") && Cookies.get("pass") != "";
    ...
    if (isLogged && amIPresenter && allowExternalVideo && !isSharingScreen) {
      actions.push({
        icon: !isSharingScreen ? "external-video" : "external-video_off",
        label: !isSharingScreen ? intl.formatMessage(intlMessages.startExternalScreenLabel)
          : intl.formatMessage(intlMessages.stopExternalScreenLabel),
        key: "external-video",onClick: () => {
          if (Session.equals('isExternalIsEnabled', true)) {
            layoutContextDispatch({
              type: ACTIONS.SET_HAS_EXTERNAL_SCREEN,
              value: false,
            });
            Session.set('isExternalIsEnabled', false);
          }else{
            layoutContextDispatch({
              type: ACTIONS.SET_HAS_EXTERNAL_SCREEN,
              value: true,
            });
            Session.set('isExternalIsEnabled', true);
          }
        },
        dataTest: "shareExternalScreen",
      })
    }
