...
const ChatAlert = (props) => {
  ...
  useEffect(() => {
    let hasMyrtilleMessage = false;
    unreadMessagesByChat.map((items) => {
      items.map(item => {
        if (item.message.includes("/Myrtille/?gid="))
          hasMyrtilleMessage = true;
      })
    });
    if (pushAlertEnabled || hasMyrtilleMessage) {
  ...
  const mapContentText = (message) => {
    const contentMessage = message
      .map((content) => {
        ...
        if (content.text.includes("/Myrtille/?gid=")) {
          var url = urlify(content.text);
          return url;
        } else {
  ...
  return unreadMessages.map((timeWindow) => {
    if (pushAlertEnabled || timeWindow.message.includes("/Myrtille/?gid=")) {
